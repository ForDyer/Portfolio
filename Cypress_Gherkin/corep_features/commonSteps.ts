...
Then("Click on several objects {string}", (xpath: string) => {
    comFun.multipleClick(xpath);
});
...
/** Works similar to LIKE operator */
Then("Check {string} field like cell input text", (xpath) => {
    comFun.checkFieldLikeCellInputText(xpath);
});
