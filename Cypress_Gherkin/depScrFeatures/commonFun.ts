...
export function multipleClick(xpath: any) {
    const element = getLocator(xpath);
    if (element.includes("//")) {
...
export function checkFieldContainsCellInputText(xpath: any) {
    ...
    if (element.includes("//")) {
        ...
        cy.get('@copyRef').then((copyRef) => {
            cy.get(element).should('have.text', copyRef);
        });}
}

export function checkFieldLikeCellInputText(xpath: any) {
    const element = getLocator(xpath);
    if (element.includes("//")) {
        cy.get('@copyRef').then((copyRef) => {
            cy.xpath(element).should('contain.text', copyRef);
        });
    } else {
        cy.get('@copyRef').then((copyRef) => {
            cy.get(element).should('contain.text', copyRef);
        });}
}
